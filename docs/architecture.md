...
- 答案路径的反方审阅也只通过 `acad-pro`。模型负责给出基于证据的反馈，版本化服务端量规负责决定是否必须修订；模型不能伪造现实证据，也不能越权把未达标路径标记完成。
...
- v1.1 的路径报名、基线诊断、现实证据、版本化产物、量规评价、现实结果和能力证据均通过 `AcademiaRepository` 持久化。D1/Drizzle 与 PostgreSQL/Prisma 保持等价语义，现有账户和学习数据不被覆盖。
